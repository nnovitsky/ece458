...
class InstrumentList(ListAPIView):
    ...
    expiration_expression = F('most_recent_cal') + F('cal_freq')
    expiration_wrapped_expression = ExpressionWrapper(expiration_expression, DateField())
    queryset = Instrument.objects.annotate(most_recent_cal=Max('calibrationevent__date')).annotate(
        ...
        calibration_expiration_date=Case(When(item_model__calibration_frequency__lte=0, then=None),
                                         When(most_recent_cal__isnull=False, then=expiration_expression),
                                         default=today,
