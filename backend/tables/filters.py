...
class InstrumentFilter(django_filters.rest_framework.FilterSet):
    ...
    class Meta:
        model = Instrument
        fields = ['vendor', 'model_number', 'comment', 'serial_number']


class CalibrationEventFilter(django_filters.rest_framework.FilterSet):
    vendor = django_filters.CharFilter(field_name='instrument__vendor', lookup_expr='icontains')
    model_number = django_filters.CharFilter(field_name='instrument__model_number', lookup_expr='icontains')
