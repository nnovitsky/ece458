...
class InstrumentTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        CalibrationEvent(instrument=Instrument.objects.all()[0], date='2020-12-31', comment="cal", user=cls.user_staff).save()
        CalibrationEvent(instrument=Instrument.objects.all()[1], date='2019-12-31', user=cls.user_non_staff).save()
        cls.cal_event_data = {
            "instrument": 1,
            "date": "2021-01-10",
            "comment": "my comment",
    ...
    def test_get_single_cal_event(self):
        pk = CalibrationEvent.objects.all()[0].pk
    ...
    def test_cal_event_filter_user(self):
        username = "newUser2"
        response = self.client.get(reverse('calibration_event_search'), {'user': username, 'get_all': True},
                                   HTTP_AUTHORIZATION='JWT {}'.format(self.token_staff), content_type='application/json')
    ...
    def test_cal_event_filter_instrument(self):
        ins_pk = Instrument.objects.all()[0].pk
        response = self.client.get(reverse('calibration_event_search'), {'instrument_pk': ins_pk, 'get_all': True},
                                   HTTP_AUTHORIZATION='JWT {}'.format(self.token_staff), content_type='application/json')
